(function(){
  ...
  const config={
    m9_aop_sernap:{name:'AOPs (SERNAP)',color:'#fccbc9',keys:['aop_total'],labels:['AOP registradas'],units:['']},
    m9_construcciones:{name:'Construcciones (Viviendas)',color:'#ff0000',keys:['viviendas_num','viviendas_area_ha'],labels:['Construcciones','Superficie total'],units:['','ha']},
    m9_vias_caminos:{name:'Vías y Caminos',color:'#e66914',keys:['caminos_principales_km','caminos_secundarios_terciarios_km'],labels:['Carreteras principales','Caminos secundarios o terciarios'],units:['km','km']},
    m9_lineas_mt:{name:'Líneas de Media Tensión',color:'#c3fcb8',keys:['lineas_mt_km'],labels:['Longitud'],units:['km']},
    m9_lineas_at:{name:'Líneas de Alta Tensión',color:'#dad4fc',keys:['lineas_at_km'],labels:['Longitud'],units:['km']},
    m9_plantas:{name:'Plantas de generación eléctrica',color:'#bafcc9',keys:['plantas_num'],labels:['Plantas'],units:['']},
    m9_represas:{name:'Represas',color:'#e3fcbd',keys:['represas_num'],labels:['Represas'],units:['']}
  };
  let focusId='m9_aop_sernap';

  const style=document.createElement('style');
  style.textContent=`
    .m9-summary-list{display:flex;flex-direction:column;gap:6px}.m9-summary-row{display:grid;grid-template-columns:10px minmax(0,1fr) auto;gap:8px;align-items:center;font-size:10px;padding:6px 0;border-bottom:1px solid var(--border)}
    .m9-summary-row:last-child{border-bottom:0}.m9-summary-row i{width:9px;height:9px;border-radius:2px}.m9-summary-row span{line-height:1.25}.m9-summary-row b{font:600 10px 'JetBrains Mono',monospace;white-space:nowrap}
  ...
  function scopedRows(){
    ...
    return selected.size===0?data.summary:data.summary.filter(row=>selected.has(norm(row.area_protegida)));
  }
  function scopedSectorRows(){
    const selected=selectedKeys();
    return selected.size===0?data.sectors:data.sectors.filter(row=>selected.has(norm(row.area_protegida)));
  }
  ...
  function currentFocus(){
    ...
    if(!active.includes(focusId)) focusId=active.at(-1)||'';
    return focusId;
  }
  ...
  function detailTable(id,rows){
    ...
    const item=config[id];
    if(!item) return {title:'Áreas protegidas del módulo',head:['Área protegida'],rows:rows.map(row=>[row.area_protegida])};
    return {title:`${item.name} por área protegida`,head:['Área protegida',...item.labels],rows:rows.map(row=>[row.area_protegida,...item.keys.map((key,index)=>format(row[key],item.units[index]))]).filter(row=>row.slice(1).some(value=>!/^0(?:[,.]00)?(?: km| ha)?$/.test(value)))};
  }
  function renderTable(spec){
    const card=document.getElementById('breakdownCard');
    if(!card)return;
    card.innerHTML=`<h4>${escapeHTML(spec.title)} <span class="mono" style="color:var(--text-dim);font-size:9px">Excel consolidado</span></h4><div style="max-height:330px;overflow:auto;margin-top:4px">${spec.rows.length?`<table class="m9-table"><thead><tr>${spec.head.map(value=>`<th>${escapeHTML(value)}</th>`).join('')}</tr></thead><tbody>${spec.rows.map(row=>`<tr>${row.map(value=>`<td>${escapeHTML(String(value))}</td>`).join('')}</tr>`).join('')}</tbody></table>`:'<div class="m9-empty">No hay registros para la selección y la capa activa.</div>'}</div>`;
  }
  function renderM9DataPanel(){
    ...
    card.innerHTML=`<h4>Indicadores de capas activas <span class="mono" style="color:var(--text-dim);font-size:9px">${escapeHTML(scope)}</span></h4><div class="m9-summary-list">${active.map(id=>{
      const item=config[id];
      const value=item.keys.map((key,index)=>`${item.labels[index]}: ${format(sum(rows,key),item.units[index])}`).join(' · ');
      return `<button type="button" data-m9-focus="${id}" class="m9-summary-row" style="border-left:0;border-right:0;border-top:0;background:transparent;color:var(--text);width:100%;text-align:left;cursor:pointer"><i style="background:${item.color}"></i><span>${item.name}</span><b>${value}</b></button>`;
    }).join('')||'<div class="m9-empty">Activa una capa temática en el panel izquierdo.</div>'}</div><div class="m9-note">La tabla inferior detalla la última capa activada. El filtro superior limita simultáneamente el mapa y los indicadores.</div><a class="m9-download" href="assets/downloads/Indicadores_Areas_Protegidas.xlsx" download>Descargar tabla Excel</a>`;
    card.querySelectorAll('[data-m9-focus]').forEach(button=>button.addEventListener('click',()=>{focusId=button.dataset.m9Focus;renderM9DataPanel();}));
    renderTable(detailTable(focus,rows));
  }
  ...
  toggleLayer=function(layerId,rowEl){
    ...
    if(currentModule===MODULE_ID){
      if(activeLayers.has(layerId)&&thematicIds.has(layerId))focusId=layerId;
      renderM9DataPanel();
    }
  };
})();
